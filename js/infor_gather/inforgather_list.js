...
app.controller('myCtrl', function($scope,$http) {
    ...
    function init(){
        if(localStorage.getItem("token") == undefined ||localStorage.getItem("token") == null){
            ...
            myGrid.setHeader("编辑人,标题,地址,油田区块,油田,添加时间,最后编辑时间,详情,添加收藏");//设置表头
            myGrid.attachHeader("<input class='search' style='width: 100px' type='text' id='parame_a'>," +
                ...
                "&nbsp;" +
                "&nbsp;");
            myGrid.setInitWidths("130,200,200,100,200,200,200,65,100");//设置表格初始宽度
                ...
                callback:function(result){
                    if(result==true){
                        $http.get(basePath+"api/v1.0/like/islike?access_token="+localStorage.getItem("token")+
                            "&user_id="+localStorage.getItem("mobile")+"&type=inforgather&like_id="+$scope.this_row_id
                        ).success(function(res){
